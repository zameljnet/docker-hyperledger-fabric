...
startFabric: # create/join channel, install/instantiate cc
	make stop
	# make clean_config # Remove existing crypto-config and channel artifacts.
	make gen_config  # Will ignore if local config path exists
	...
	@echo "Now the fabric network is ready to play"
	@echo "* run 'make cli' to enter into the fabric-cli container."
	@echo "* run 'make stop clean' when done."
...
logs_save: # save logs
	...
	docker logs peer1.org1.example.com >& $(LOG_PATH)/dev_peer1.log
	docker logs peer0.org2.example.com >& $(LOG_PATH)/dev_peer2.log
	docker logs peer1.org2.example.com >& $(LOG_PATH)/dev_peer3.log
